              ...
              <div className="space-y-4">
                {isLoading ? (
                  ...
                  addresses.map((addr, idx) => (
                    <div
                      key={addr._id || idx}
                      className={`p-5 rounded-2xl border transition-all relative flex items-start gap-4 ${addr.is_default
                          ? 'bg-[#F4F9F5] border-[#0C3823]/30 shadow-xs'
                          : 'bg-[#FAFBF9] border-slate-100 hover:border-slate-200'
                        }`}
                    >
                      <div className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${addr.is_default ? 'bg-[#0C3823] text-white' : 'bg-white text-slate-500 border border-slate-200'
                        }`}>
                        <FiMapPin size={20} />
                      </div>
