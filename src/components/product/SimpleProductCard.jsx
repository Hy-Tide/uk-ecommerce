      ...
      <div className="relative bg-[#f9f9f9] rounded-lg pt-[100%] overflow-hidden mb-3">
        ...
        <Link to={getProductUrl(product)} className="absolute inset-0 w-full h-full flex items-center justify-center p-2">
          {imageUrl && !imgError ? (
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-slate-50/80 rounded-lg p-2 select-none text-center">
              <img
                src="/images/logo.png"
                alt="Grandma's Basket Logo"
                className="w-10 h-10 object-contain opacity-25 grayscale hover:grayscale-0 hover:opacity-40 transition-all duration-300"
              />
              <span className="text-[9px] text-slate-400 font-bold mt-1 opacity-70">
                Grandma's Basket
              </span>
            </div>
          )}
