      ...
      <div className="container px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
          {categories.map((cat, index) => (
            ...
            >
              <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl mb-4 group-hover:scale-110 transition-transform overflow-hidden ${cat.color || 'bg-emerald-100 text-emerald-600'}`}>
                {cat.image ? (
              ...
              {cat.count !== undefined && (
                <p className="text-sm text-slate-400 font-medium">{cat.count} {cat.count === 1 ? 'Article' : 'Articles'}</p>
              )}
            </motion.a>
          ))}
        </div>
